...
Goblin.registerQuest(goblinName, 'close', function* (quest, sessionDesktopId) {
  ...
  yield quest.me.killSession({sessionDesktopId});
});

Goblin.registerQuest(goblinName, 'killSession', function* (
  quest,
  sessionDesktopId
) {
  yield quest.warehouse.feedSubscriptionDel({
    feed: sessionDesktopId,
    branch: sessionDesktopId,
    parents: sessionDesktopId,
  });
...
function* _postload(msg, resp, next) {
  try {
    yield resp.command.nestedSend('desktop-manager.init', null, next);
